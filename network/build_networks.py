...
from network.DPRRN import DPRRN
...
def get_scheduler(optimizer, opt):
    if opt.lr_policy == 'linear':
        ...
        scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[220, 340, 460], gamma=0.2)
    elif opt.lr_policy == 'multistep_warm':
        scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[245, 365, 485], gamma=0.2)
    elif opt.lr_policy == 'multistep_finetune':
        scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[230, 250, 270], gamma=0.2)
...
def define_G(netG, init_type='normal', init_gain=0.02, gpu_ids=None, init=True):
    if gpu_ids is None:
        gpu_ids = []
