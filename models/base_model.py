    ...
    def load_networks(self):
        ...
        for name in self.model_names:
            if isinstance(name, str):
                ...
                if self.continue_train or not self.isTrain:
                    ...
                    print(list(state_dict.keys())[:3], list(ori_state_dict.keys())[:3])
                    for key in state_dict.keys():
                        new_key = key.replace('module.', '')
                        if new_key in ori_state_dict:
                            if ori_state_dict[new_key].size() == state_dict[key].size():
                                print(f'{new_key} is in ori_state_dict.keys(), loading...')
                                state_dict_new[new_key] = state_dict[key]
                    if hasattr(state_dict_new, '_metadata'):
                        del state_dict_new._metadata
                    ori_state_dict.update(state_dict_new)
