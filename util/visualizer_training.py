...
class Visualizer():
    ...
    def __init__(self, opt):
        ...
        train_log_dir = os.path.join('logs\\tensorboard\\image_deraining\\train', opt.name)
        print('log dir train:', train_log_dir)

        val_log_dir = 'logs\\tensorboard\\image_deraining\\val\\%s' % opt.name.replace('/', '')
        print('log dir val:', val_log_dir)

        self.train_summary_writer = summary.create_file_writer(train_log_dir)
        self.validation_summary_writer = summary.create_file_writer(val_log_dir)
    ...
    def plot_current_losses(self, epoch, losses, cur_iter=None, is_epoch_loss=False,is_val=False):
        if not is_val:
    ...
    def print_current_losses(self, epoch, losses, iters=None, t_comp=None, t_data=None, is_epoch_loss=False,is_val=False):
        if is_epoch_loss:
            message = '(Epoch total loss: %d ' % epoch
        elif is_val:
            message = '(Epoch total loss in Validation: %d ' % epoch
        else:
